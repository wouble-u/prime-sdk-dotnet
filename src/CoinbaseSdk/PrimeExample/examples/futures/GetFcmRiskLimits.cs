...
var rootCommand = new RootCommand("Get FCM risk limits for an entity")
{
    entityIdOption,
};
...
rootCommand.SetHandler((entityId) =>
{
    ...
    try
    {
        ...
        var request = new GetFcmRiskLimitsRequest(entityId);

        PrettyPrinter.PrintResponse("GetFcmRiskLimitsRequest", request);
    ...
    {
        PrettyPrinter.PrintError("Error getting FCM risk limits", ex);
        Environment.ExitCode = 1;
    }
}, entityIdOption);
