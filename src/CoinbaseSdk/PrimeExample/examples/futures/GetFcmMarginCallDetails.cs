...
var rootCommand = new RootCommand("Get FCM margin call details for an entity")
{
    entityIdOption,
};
...
rootCommand.SetHandler((entityId) =>
{
    ...
    try
    {
        ...
        var request = new GetFcmMarginCallDetailsRequest(entityId);

        PrettyPrinter.PrintResponse("GetFcmMarginCallDetailsRequest", request);

        var response = futuresService.GetFcmMarginCallDetails(request);

        PrettyPrinter.PrintResponse("GetFcmMarginCallDetailsResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error getting FCM margin call details", ex);
        Environment.ExitCode = 1;
    }
}, entityIdOption);
