...
var rootCommand = new RootCommand("Update Onchain Address Book Entry")
{
    portfolioIdOption,
    addressGroupOption,
};
...
rootCommand.SetHandler((context) =>
{
    ...
    try
    {
        ...
        PrettyPrinter.PrintResponse("UpdateOnchainAddressBookEntryResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error calling UpdateOnchainAddressBookEntry", ex);
        Environment.ExitCode = 1;
    }
});
