...
var rootCommand = new RootCommand("Get balance for a specific wallet")
{
    portfolioIdOption,
    walletIdOption
};
...
rootCommand.SetHandler((portfolioId, walletId) =>
{
    ...
    try
    {
        ...
        var request = new GetWalletBalanceRequest(portfolioId, walletId);

        PrettyPrinter.PrintResponse("GetWalletBalanceRequest", request);

        var response = balancesService.GetWalletBalance(request);

        PrettyPrinter.PrintResponse("GetWalletBalanceResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error getting wallet balance", ex);
        Environment.ExitCode = 1;
    }
