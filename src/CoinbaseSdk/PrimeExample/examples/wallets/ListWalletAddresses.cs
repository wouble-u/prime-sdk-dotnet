...
var networkIdOption = new Option<string?>(
    name: "--networkId",
    description: "Optional network ID to filter addresses");

var rootCommand = new RootCommand("List addresses for a wallet")
{
    portfolioIdOption,
    walletIdOption,
    networkIdOption,
...
rootCommand.SetHandler((portfolioId, walletId, networkId) =>
{
    ...
    try
    {
        ...
        // Build request
        var request = new ListWalletAddressesRequest(portfolioId, walletId);
        ...
        PrettyPrinter.PrintResponse("ListWalletAddressesRequest", request);

        // Execute request
        var response = walletsService.ListWalletAddresses(request);

        // Print response
        PrettyPrinter.PrintResponse("ListWalletAddressesResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error listing wallet addresses", ex);
        Environment.ExitCode = 1;
    }
