...
var rootCommand = new RootCommand("Get a specific wallet")
{
    portfolioIdOption,
    walletIdOption,
};
...
rootCommand.SetHandler((portfolioId, walletId) =>
{
    ...
    try
    {
        ...
        // Build request
        var request = new GetWalletRequest(portfolioId, walletId);

        PrettyPrinter.PrintResponse("GetWalletRequest", request);

        // Execute request
        var response = walletsService.GetWallet(request);

        // Print response
        PrettyPrinter.PrintResponse("GetWalletResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error getting wallet", ex);
        Environment.ExitCode = 1;
    }
