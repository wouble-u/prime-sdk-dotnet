...
var rootCommand = new RootCommand("Create an allocation")
{
...
rootCommand.SetHandler((context) =>
{
    ...
    try
    {
        ...
        var request = new CreateAllocationRequest.Builder()
        ...
        PrettyPrinter.PrintResponse("CreateAllocationRequest", request);

        var response = allocationsService.CreateAllocation(request);

        PrettyPrinter.PrintResponse("CreateAllocationResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error creating allocation", ex);
        Environment.ExitCode = 1;
    }
});
