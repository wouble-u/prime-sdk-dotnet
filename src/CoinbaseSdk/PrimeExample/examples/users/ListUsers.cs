...
var rootCommand = new RootCommand("List users for an entity")
{
    entityIdOption
};
...
rootCommand.SetHandler((entityId) =>
{
    ...
    try
    {
        ...
        // Build request
        var request = new ListUsersRequest(entityId);

        PrettyPrinter.PrintResponse("ListUsersRequest", request);

        // Execute request
        var response = usersService.ListUsers(request);

        // Print response
        PrettyPrinter.PrintResponse("ListUsersResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error listing users", ex);
        Environment.ExitCode = 1;
    }
}, entityIdOption);
