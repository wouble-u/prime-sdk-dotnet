...
var rootCommand = new RootCommand("Get Staking Status")
{
    portfolioIdOption,
    walletIdOption,
};
...
rootCommand.SetHandler((context) =>
{
    ...
    try
    {
        ...
        var request = new GetStakingStatusRequest(portfolioId, walletId);

        PrettyPrinter.PrintResponse("GetStakingStatusRequest", request);

        var response = stakingService.GetStakingStatus(request);

        PrettyPrinter.PrintResponse("GetStakingStatusResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error calling GetStakingStatus", ex);
        Environment.ExitCode = 1;
    }
});
