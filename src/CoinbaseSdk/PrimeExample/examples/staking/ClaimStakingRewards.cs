...
var rootCommand = new RootCommand("Claim staking rewards from a wallet")
{
    portfolioIdOption,
    walletIdOption
};
...
rootCommand.SetHandler((portfolioId, walletId) =>
{
    ...
    try
    {
        ...
        var request = new ClaimStakingRewardsRequest(portfolioId, walletId)
        {
            IdempotencyKey = Guid.NewGuid().ToString()
        };

        PrettyPrinter.PrintResponse("ClaimStakingRewardsRequest", request);

        var response = stakingService.ClaimStakingRewards(request);

        PrettyPrinter.PrintResponse("ClaimStakingRewardsResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error claiming staking rewards", ex);
        Environment.ExitCode = 1;
    }
