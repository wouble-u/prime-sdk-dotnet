...
var amountOption = new Option<string?>(
    name: "--amount",
    description: "Amount to unstake (optional, defaults to max available)");

var rootCommand = new RootCommand("Create a wallet-level unstake")
{
    portfolioIdOption,
    walletIdOption,
    amountOption
...
rootCommand.SetHandler((portfolioId, walletId, amount) =>
{
    ...
    try
    {
        ...
        var inputsBuilder = new WalletUnstakeInputs.Builder();
        if (!string.IsNullOrEmpty(amount))
        {
            inputsBuilder.WithAmount(amount);
        }

        var request = new CreateUnstakeRequest(portfolioId, walletId)
        {
            IdempotencyKey = Guid.NewGuid().ToString(),
            Inputs = inputsBuilder.Build()
        };

        PrettyPrinter.PrintResponse("CreateUnstakeRequest", request);

        var response = stakingService.CreateUnstake(request);

        PrettyPrinter.PrintResponse("CreateUnstakeResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error creating unstake", ex);
        Environment.ExitCode = 1;
    }
