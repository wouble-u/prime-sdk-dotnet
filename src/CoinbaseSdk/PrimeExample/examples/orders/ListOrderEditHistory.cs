...
var orderIdOption = new Option<string?>(
    name: "--orderId",
    description: "The Order ID to get edit history for");

var rootCommand = new RootCommand("List edit history for a specific order")
{
    portfolioIdOption,
    orderIdOption,
};
...
rootCommand.SetHandler((context) =>
{
    ...
    try
    {
        ...
        Console.WriteLine($"Getting edit history for Order ID: {orderId}");

        var client = CoinbasePrimeClient.FromEnv();
        var ordersService = new OrdersService(client);

        var request = new ListOrderEditHistoryRequest(portfolioId, orderId);

        PrettyPrinter.PrintResponse("ListOrderEditHistoryRequest", request);

        var response = ordersService.ListOrderEditHistory(request);

        PrettyPrinter.PrintResponse("ListOrderEditHistoryResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error listing order edit history", ex);
        Environment.ExitCode = 1;
    }
});
