...
var rootCommand = new RootCommand("List all fills for a portfolio")
{
    portfolioIdOption,
    startDateOption,
    endDateOption,
...
rootCommand.SetHandler((context) =>
{
    ...
    try
    {
        ...
        var request = new ListPortfolioFillsRequest(portfolioId);
        ...
        PrettyPrinter.PrintResponse("ListPortfolioFillsRequest", request);

        var response = ordersService.ListPortfolioFills(request);

        PrettyPrinter.PrintResponse("ListPortfolioFillsResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error listing portfolio fills", ex);
        Environment.ExitCode = 1;
    }
});
