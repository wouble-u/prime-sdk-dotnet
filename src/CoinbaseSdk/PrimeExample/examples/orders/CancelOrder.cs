...
var rootCommand = new RootCommand("Cancel an existing order")
{
    portfolioIdOption,
    orderIdOption,
};
...
rootCommand.SetHandler((context) =>
{
    ...
    try
    {
        ...
        Console.WriteLine($"Cancelling Order ID: {orderId}");

        var client = CoinbasePrimeClient.FromEnv();
        var ordersService = new OrdersService(client);

        var request = new CancelOrderRequest(portfolioId, orderId);

        PrettyPrinter.PrintResponse("CancelOrderRequest", request);

        var response = ordersService.CancelOrder(request);

        PrettyPrinter.PrintResponse("CancelOrderResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error cancelling order", ex);
        Environment.ExitCode = 1;
    }
});
