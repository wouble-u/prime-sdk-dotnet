...
var rootCommand = new RootCommand("List Advanced Transfers")
{
    portfolioIdOption,
};
...
rootCommand.SetHandler((portfolioId) =>
{
    ...
    try
    {
        ...
        var request = new ListAdvancedTransfersRequest(portfolioId);

        PrettyPrinter.PrintResponse("ListAdvancedTransfersRequest", request);

        var response = advancedTransferService.ListAdvancedTransfers(request);

        PrettyPrinter.PrintResponse("ListAdvancedTransfersResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error calling ListAdvancedTransfers", ex);
        Environment.ExitCode = 1;
    }
}, portfolioIdOption);
