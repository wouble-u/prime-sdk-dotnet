...
var rootCommand = new RootCommand("Get Transaction Travel Rule Data")
{
    portfolioIdOption,
    transactionIdOption,
};
...
rootCommand.SetHandler((context) =>
{
    ...
    try
    {
        ...
        var request = new GetTransactionTravelRuleDataRequest(portfolioId, transactionId);

        PrettyPrinter.PrintResponse("GetTransactionTravelRuleDataRequest", request);

        var response = transactionsService.GetTransactionTravelRuleData(request);

        PrettyPrinter.PrintResponse("GetTransactionTravelRuleDataResponse", response);

        Environment.ExitCode = 0;
    }
    catch (Exception ex)
    {
        PrettyPrinter.PrintError("Error calling GetTransactionTravelRuleData", ex);
        Environment.ExitCode = 1;
    }
});
